...
void display_updater() {
  display.display(30); // Increase refresh rate (reduce the interval)
}
...
void scroll_text(uint8_t ypos, unsigned long scroll_delay, String text) {
  ...
  for (int xpos = 170; xpos > -(32 + text_length * 10); xpos--) { // Adjusted for larger text size
    ...
    display.fillRect(xpos, ypos, (text_length + 5) * 10, 16, 0); // Clear previous text with black color
  ...
  // Clear the entire scrolling section after the loop ends
  display.fillRect(0, ypos, display.width(), 16, 0); // Clear the entire scrolling section
